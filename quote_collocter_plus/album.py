...
def effective_album_settings(
    ...
    return AlbumSettings(
        ...
        album_name=get_str("album_name", plugin_settings.album_name),
...
class AlbumService:
    ...
    def get_effective_settings(self, group_settings: dict[str, Any] | None) -> AlbumSettings:
        return effective_album_settings(self.settings, group_settings)
    ...
    async def result_suffix(self, event: Any, group_context: GroupContext, image_path: str) -> str:
        settings = self.get_effective_settings(group_context.admin_settings)
    ...
    async def handle_command(self, event: Any, group_context: GroupContext, msg: str) -> str:
        ...
        if args in {"", "状态"}:
            return format_album_upload_status(
                self.get_effective_settings(group_context.admin_settings),
            )
